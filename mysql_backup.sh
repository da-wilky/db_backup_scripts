...
TABLES=$(docker compose -f "$COMPOSE_FILE" exec -T "$COMPOSE_SERVICE" mysql -u "$DB_USER" -p"$DB_PASSWORD" -D "$DB_NAME" -e "SHOW TABLES;" | tail -n +2)
...
for TABLE in $TABLES; do
    ...
    if [ -n "$TABLE" ]; then
        ...
        docker compose -f "$COMPOSE_FILE" exec -T "$COMPOSE_SERVICE" mysqldump -u "$DB_USER" -p"$DB_PASSWORD" "$DB_NAME" "$TABLE" > "$OUTPUT_DIR/$TABLE.sql"
        if [ $? -eq 0 ]; then
